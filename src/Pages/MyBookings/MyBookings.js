...
const MyBookings = () => {
    ...
    useEffect(() => {
        async function myBooking() {
            await axios.get(`https://tourism-backend-api.herokuapp.com/my-bookings/${user.email}`).then(res => {
                setBooking(res.data);
            });
        }
        myBooking();
    ...
    const handleDeleteBooking = (bookingId) => {
        async function deleteBooking() {
            await axios.delete(`https://tourism-backend-api.herokuapp.com/delete-booking/${bookingId}`).then(res => {
                setMassage(res.data);
            });
        }
        deleteBooking();
                    ...
                    <div className="modal-content bg-color-main">
                        ...
                        {deletingItem ?
                            <div className="modal-footer">
                                ...
                                <button type="button" className="main-button" data-dismiss="modal" onClick={() => { handleDeleteBooking(deletingItem._id) }}>Sure</button>
                            </div> : <></>}
                    </div>
                </div>
            </div>
