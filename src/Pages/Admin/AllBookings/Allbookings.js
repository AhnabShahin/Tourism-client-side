...
const AllBookings = () => {
    ...
    async function myBooking() {
        await axios.get(`https://tourism-backend-api.herokuapp.com/all-bookings`).then(res => {
            setBooking(res.data);
        });
    }
    useEffect(() => {
    ...
    const handleStatusChange = (bookingId) => {
        async function updateStatus() {
            await axios.put(`https://tourism-backend-api.herokuapp.com/update-booking-status/${bookingId}/${value}`).then(res => {
                setMassage(res.data);
            });
        }
        updateStatus();
